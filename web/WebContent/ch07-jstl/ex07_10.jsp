...
<c:choose>
	...
	<h2>점검하려는 버스의 번호를 입력하세요.</h2>
		<hr>
		<form method="post" action="ex07_10.jsp">
			버스번호 : <input type="number" name="num" min="0" required="required"/>
		...
		<c:otherwise>
			...
			<x:forEach select="$xdata//itemList" >
				...
				<c:if test="${empty outFlag }">
					<h3>${param.num }번 버스의 정보는 존재하지 않습니다.</h3>
				</c:if>
				</x:if>
				<x:if select="$xdata//headerCd != 0 ">
					<h3>${param.num }번 버스의 정보는 존재하지 않습니다.</h3>
